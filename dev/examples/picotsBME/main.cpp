...
#define add2byte(val)                 \
    {                                 \
        message[ind] = lowByte(val);  \
        ind++;                        \
        message[ind] = highByte(val); \
        ind++;                        \
    }
#define add4byte(val)                      \
    {                                      \
        message[ind] = lowByte(val);       \
        ind++;                             \
        message[ind] = lowByte(val >> 8);  \
        ind++;                             \
        message[ind] = lowByte(val >> 16); \
        ind++;                             \
        message[ind] = lowByte(val >> 24); \
        ind++;                             \
    }
...
void mainPicot() {
    ...
    int16_t adsBatt = batvolt;
    float bme_t = 0, bme_p = 0, bme_u = 0;
    cap.bmeSetup();
    bool btake = cap.bme.takeForcedMeasurement();
    if (btake) {
        bme_t = cap.bme.readTemperature() * 10;
        bme_p = cap.bme.readPressure() / 100.0;
        bme_u = cap.bme.readHumidity() * 10;
    }
    Serial.println("temperature " + String((int)(bme_t / 10)) + "*C");
    Serial.println("pressure " + String(bme_p) + "kPa");
    Serial.println("humidity " + String((int)(bme_u / 10)) + "%");

    pinMode(pins.battPin, INPUT);
    bool as = cap.adsSetup();
    if (as) {
        adsBatt = cap.ads1015.readADC_SingleEnded(2);
    }
    Serial.println("battery " + String(adsBatt * 2.0 / 1000.0) + "V");

    if (!btake) {
        rtc.log(adsBatt, waitingtrans, w);
    } else {
        rtc.logBME(adsBatt, waitingtrans, w, bme_t, bme_p, bme_u);
    }
