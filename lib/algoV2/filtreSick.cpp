...
void filtreSick::init(float ratioplage, long per, int sampleper) {
    // per = periode picot
    // sampleper = 1200 arbitraire
    // ratioplage = 0.8 arbitraire
    float plage = (float)per / (float)sampleper * ratioplage; // plage = per / 1500
    rp = ratioplage;                                          // save rp in filtre memory
    int iplage = (int)std::floor(plage);                      // int plage
    if (iplage % 2 == 1) {                                    // demiplage for even or odd int
        demiplage = (iplage - 1) / 2;
    } else {
        demiplage = iplage / 2;
    }
    period0 = per;            // save period in filtre memory
    sampleperiod = sampleper; // save sampleperiod in filtre memory
    float A[25];
    for (int i = 0; i < ncor; i++) { // init A and AM (5x5 matrix)
        for (int j = 0; j < ncor; j++) {
            A[ncor * i + j] = 0;
            AM[ncor * i + j] = 0;
        }
    }
    for (int k = -demiplage; k <= demiplage; k++) { // magie coef pour Cholesky
...
int filtreSick::newVal(int V, unsigned long t) {
    vnew = V;
    tnew = t;
    if (count == 0) {
        vold = (float)V;
